...
def postpone_appointment():
    # Fetch appointment ID from the query parameters
    ...
    if not appointment_id:
        return print('I dont think its in object format')

    # Find the appointment by ID
    appointment = mongo.db.appointments.find_one({"_id": ObjectId(appointment_id)})
    if not appointment:
        return print('its not in the database')

    doctor_id = appointment['doctor_id']
    current_date = appointment['date']
    # Find the next available date with an available time slot
    next_available_date = mongo.db.availability.find_one({
        "doctor_id": ObjectId(doctor_id),
        "date": {"$gt": current_date},  # Find dates greater than the current date
        "is_available": True
    }, sort=[("date", 1)])  # Sort by date to find the earliest available
    print('we have a next date')
    if next_available_date:
        available_time_slots = [
            time for index, time in enumerate(next_available_date['time_slot'])
            if next_available_date['is_available'][index]
        ]
        print('These are the next available time slots:', available_time_slots)
        print('This is the next available date:', next_available_date)
        
        if available_time_slots:
            next_time_slot = available_time_slots[0]  # Get the first available time slot
    ...
    return jsonify({'success': False, 'message': 'No available slots found.'})
...
def book_appointment():
    ...
    time_slots = availability_doc.get("time_slot", [])  # Adjust the field name if different
